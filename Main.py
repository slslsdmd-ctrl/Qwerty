...
async def coin_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == result:
        win = bet * 2
        msg = f"🎉 ПОБЕДА! ВЫПАЛ {result}! +{round_ton(win)} TON (x2)"
    else:
        win = -bet
...
async def number_bet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if guess == number:
        win = bet * 3
        msg = f"🎉 ПОБЕДА! ЗАГАДАНО {number}! +{round_ton(win)} TON (x3)"
    else:
        win = -bet
...
async def dice_sum(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if guess == total:
        win = bet * 3
        msg = f"🎉 ПОБЕДА! {d1}+{d2}={total}! +{round_ton(win)} TON (x3)"
    else:
        win = -bet
    ...
    users[uid]["spins"] += 1
    save_data()
    await update.message.reply_text(f"🎲 КОСТИ (СУММА) (x3)\n\nСТАВКА: {bet} TON\n{msg}\n💵 НОВЫЙ БАЛАНС: {round_ton(users[uid]['balance'])} TON", reply_markup=main_menu())
...
async def dice_over(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (choice == "больше" and total > 7) or (choice == "меньше" and total < 7):
        win = bet * 2
        msg = f"🎉 ПОБЕДА! {d1}+{d2}={total}! +{round_ton(win)} TON (x2)"
    elif total == 7:
        win = 0
    ...
    users[uid]["spins"] += 1
    save_data()
    await update.message.reply_text(f"🎲 КОСТИ (БОЛЬШЕ/МЕНЬШЕ) (x2)\n\nСТАВКА: {bet} TON\n{msg}\n💵 НОВЫЙ БАЛАНС: {round_ton(users[uid]['balance'])} TON", reply_markup=main_menu())
...
async def dice_even(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (choice == "чет" and is_even) or (choice == "нечет" and not is_even):
        win = bet * 2
        msg = f"🎉 ПОБЕДА! {d1}+{d2}={total} ({choice})! +{round_ton(win)} TON (x2)"
    else:
        win = -bet
        msg = f"😢 ПРОИГРЫШ! {d1}+{d2}={total}... -{round_ton(bet)} TON"
    users[uid]["balance"] = round_ton(users[uid]["balance"] + win)
    ...
    users[uid]["spins"] += 1
    save_data()
    await update.message.reply_text(f"🎲 КОСТИ (ЧЁТ/НЕЧЕТ) (x2)\n\nСТАВКА: {bet} TON\n{msg}\n💵 НОВЫЙ БАЛАНС: {round_ton(users[uid]['balance'])} TON", reply_markup=main_menu())
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🔢 /number [СУММА] [1-10] - Угадай число (x3)\n"
        f"🎲 /dice_sum [СУММА 2-12] [БЕТ] - Кости сумма (x3)\n"
        f"🎲 /dice_over [больше/меньше] [БЕТ] - Кости больше/меньше 7 (x2)\n"
        f"🎲 /dice_even [чет/нечет] [БЕТ] - Кости чёт/нечет (x2)\n\n"
